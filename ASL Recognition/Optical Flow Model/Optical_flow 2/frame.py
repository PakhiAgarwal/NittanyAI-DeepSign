...
def video_length(path:str) -> float:
    newPath = path + '.webm'
    os.system('ffmpeg -i ' + path + ' -vcodec copy -acodec copy -y ' + newPath)
    vLength = int(check_output(["mediainfo", '--Inform=General;%Duration%', newPath])) / 1000.0
...
def videosDir2framesDir(sVideoDir:str, sFrameDir:str, nFramesNorm:int = None, 
    ...
    """
    # nFramesNorm: the number of extracted frames
    # nResizeMinDim: The mini width of the frames
    # tuCropShap: use a tuple to pass height and weight
    # nClasses: the number of classes/pictures
...
def unittest(sVideoDir, nSamples = 100):
    ...
    liVideos = glob.glob(sVideoDir + "/*/*.webm")
...
if __name__ == '__main__':
    ...
    #unittest("/DSResources/Test", 3)
    videosDir2framesDir("/DSResources/SecondRound","/DSResources/Output", 10, 256, None, 121)
